...
#template-deployment-centos
EXFILENAME=$SUFFIX_YAML'deployment-centos.yaml'
cp -f $EXPATH/template_yamls/template-deployment-centos.yaml $EXPORT_DIR/$EXFILENAME
sed -i -e "s/{{ DEPLOYMENT.NAME }}/$DEPLOYMENT_CENTOS_NAME/g" $EXPORT_DIR/$EXFILENAME
sed -i -e "s/{{ DEPLOYMENT.NAMESPACE }}/$NAMESPACE/g" $EXPORT_DIR/$EXFILENAME
sed -i -e "s/{{ DEPLOYMENT.APPLABEL }}/$DEPLOYMENT_CENTOS_APP_LABEL/g" $EXPORT_DIR/$EXFILENAME
...
cp -f $EXPATH/template_yamls/template-configmap.yaml $EXPORT_DIR/$EXFILENAME
